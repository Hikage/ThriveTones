...
public class NNet implements Comparable{
	public final static int INPUTS = 16;
	public final static int HIDDEN = 16;
	public final static int OUTPUTS = 2;
	public final static int ACTIVATION_MAGNITUDE = 25;
	...
	public NNet()
	{
//		System.out.println("Generating Neural Net");
	...
	public int[] calculate(int inputMelody[])
	{
		int input[] = melodyToInputs(inputMelody);
	...
	public static int[] melodyToInputs(int melody[])
	{
		int inputs[] = new int[INPUTS];
		inputs[0] = melody[0] / 12;
		for (int i = 1; i < Song.MELODY_LENGTH; i++)
		{
			inputs[i] = melody[i] - melody[i-1];
		}
		for (int j = Song.MELODY_LENGTH; j < INPUTS; j++)
		{
			inputs[j] = melody[j];
		}
		return inputs;
